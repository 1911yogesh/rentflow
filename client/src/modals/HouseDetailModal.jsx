...
const HouseDetailModal = ({ open, onClose, onSave, house: initialHouse }) => {
  ...
  const [house,    setHouse]    = useState(initialHouse);   // always the freshest copy
  ...
  const [payModal,      setPayModal]      = useState(null);  // record being paid
  const [editModal,     setEditModal]     = useState(null);  // record being edited
  const [slipRecord,    setSlipRecord]    = useState(null);  // record being viewed

  // ── Load house fresh from server and fetch records ─────────────────────────
  const loadHouse = useCallback(async (houseId) => {
    try {
  ...
  const loadRecords = useCallback(async (houseId) => {
    setLoading(true);
    try {
  ...
  // On modal open: always fetch fresh house data and records
  useEffect(() => {
  ...
  // ── Master refresh: re-fetch house + records, propagate up to Houses page ──
  const refresh = useCallback(async () => {
    const houseId = (house || initialHouse)?._id;
    if (!houseId) return;
    const freshHouse = await loadHouse(houseId);
    await loadRecords(houseId);
    onSave?.();           // tell Houses page to re-fetch its grid
    return freshHouse;
  }, [house, initialHouse, loadHouse, loadRecords, onSave]);

  // ── After editing a record in-list (payment / edit slip) ──────────────────
  // Update just that one record in the list without closing any modal.
  const updateRecordInList = useCallback((updatedRecord) => {
    setRecords(prev =>
      prev.map(r => r._id === updatedRecord._id ? updatedRecord : r)
    );
    // Also refresh house so prevReading etc. are current
  ...
  const deleteRecord = async (record) => {
    ...
    try {
      ...
      // Remove from local list instantly, then refresh house for rolled-back reading
  ...
  const deletePayment = async (record, txnId) => {
    ...
    try {
      ...
      // Update just this record in the list — don't close anything
  ...
  return (
    <>
      <Modal open={open} onClose={onClose} title="" size="lg">
          ...
          isOccupied ? (
            <div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
                ...
                <InfoCard title="Rent Config">
                  ...
                  {house.elecType === 'fixed'
                    ? <InfoRow label="Electricity" value={`₹${house.elecFixed} (Fixed)`} />
              ...
              <div className="flex gap-2 flex-wrap">
                <button className="btn btn-primary"         onClick={() => setTab('rent')}>🧾 Generate Slip</button>
                <button className="btn btn-secondary btn-sm" onClick={() => setTenantModal(true)}>✏️ Edit Tenant</button>
        ...
        {tab === 'rent' && isOccupied && (
          <div className="text-center py-8">
            ...
            <button className="btn btn-primary" onClick={() => setGenerateModal(true)}>Generate Slip</button>
          </div>
        )}
      ...
      {/* ── Sub-modals — always receive the fresh `house` state ── */}

      {/* Edit tenant: on save, refresh house + re-render info tab live */}
      <TenantModal
        open={tenantModal}
        house={house}
      ...
      {/* Generate slip: pass live house so prevReading is always current */}
      <RentSlipGenerateModal
        open={generateModal}
        house={house}
        onClose={() => setGenerateModal(false)}
        onSave={async () => {
          // Refresh house (for updated prevReading) + records list
          await refresh();
          setTab('history');   // jump to history to show the new slip
        }}
      />

      {/* Add payment: update just that record in-list, no full refresh needed */}
      <AddPaymentModal
        open={!!payModal}
        record={payModal}
        onClose={() => setPayModal(null)}
        onSave={(updatedRecord) => {
          if (updatedRecord) updateRecordInList(updatedRecord);
          else refresh();
          setPayModal(null);
        }}
      />

      {/* Edit slip: update just that record in-list */}
      <EditSlipModal
        open={!!editModal}
        record={editModal}
        ...
        <div className="mt-3 space-y-2">
          <div className="bg-white rounded-lg p-3 text-xs space-y-1">
            ...
            <BillLine label="Electricity" val={r.elecBill}
              sub={r.elecType === 'fixed' ? 'Fixed amount' : `${r.units} × ₹${r.perUnit}`} />
            {(r.previousDue?.final > 0 || r.previousDue > 0) && (
              <BillLine label="Prev Due" val={r.previousDue} red />
            )}
          </div>
            ...
            <div className="space-y-1.5">
              ...
              {r.transactions.map((txn) => (
                <div key={txn._id} className="flex items-center gap-2 bg-white rounded-lg px-3 py-2 text-xs">
                  ...
                  <button onClick={() => onDeletePayment(txn._id)}
